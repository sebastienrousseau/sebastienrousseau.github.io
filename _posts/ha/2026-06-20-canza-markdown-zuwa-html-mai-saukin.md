...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[Shigar da binciken AST]
        A1[Raw Markdown da YAML front matter] --> B(Tushen mai-sarrafa HTML Generator)
        B --> C{Iyakar sandbox na WASM}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[Ƙofofin gini masu aminci]
        C --> D{Mai bincike HTML AST}
        D -->|Ƙa'idar WCAG ta keta: kanun lebur ko rubutu alt da ya ɓace| E[Tsayar da mai-sarrafa kuma gazawar gini]
        D -->|WCAG 2.1 AA ya bi| F[Mai-sarrafa abun ciki mai arziki]
    end

    %% Hardening & Output
    subgraph Enrichment[Arzatawa da sarrafa]
        F -->|Sarrafa lissafi zuwa MathML| G[Mai haifar MathML]
        F -->|Nuna zane-zane zuwa SVG| H[Mai haifar Mermaid SVG]
        F -->|Fitar metadata| I[Mai shigo da schema na JSON-LD]
        G --> J[Fitarwar HTML mai ƙarfi, da aka rage]
        H --> J
        I --> J
    end
