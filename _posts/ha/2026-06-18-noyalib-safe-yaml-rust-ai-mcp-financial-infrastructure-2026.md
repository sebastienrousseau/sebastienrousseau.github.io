...
```mermaid
graph TD
  subgraph Raw_Manifest_Ingestion [Karɓar Manifest Mai Tsabta]
    A1[GitHub Repository / YAML 1.2] -->|1. Ɗauki Tsari| B(NoyaLib Parser)
    A2[Wakilin AI / Kayan Aikin Refactoring na Atomatik] -->|2. Ba da Shawarar Canji na Cikin Gida| B
  end
  subgraph NoyaLib_Core_Parser [NoyaLib Core Parser]
    B -->|3. Parse tare da Sifilin Unsafe Blocks| C{Lossless CST Generator}
    C -->|4. Gina CST yana kiyaye sharhi & sarari| D[Concrete Syntax Tree CST]
  end
  subgraph Schema_Validation_Gate [Ƙofar Tabbatar da Schema]
    D -->|5. Cire Abstract Syntax Tree AST| E[JSON-Schema Validator]
    E -->|Ketaren Schema / Nau'in Maras Inganci| F[Dakatar da Bututun & Ƙi Canji]
    E -->|Schema Tabbacin 100%| G[WASM Compiler / GPG Signer]
  end
  subgraph Secure_Cloud_Native_Deployment [Turawar Cloud-Native Mai Aminci]
    G -->|6. Tara YAML Tabbatacce zuwa WASM / JSON| H[Kubernetes Cluster / CI Engine]
    G -->|7. Ƙara Log Na Audit| I[Rajistan Ayyuka Mara Canzawa]
  end
```
