...
![Philip Intallura, HSBC Küresel Kuantum Teknolojileri Başkanı, Economist Impact 5. Yıllık Commercialising Quantum Global 2026 zirvesinde sahnede](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Kriptografik Keşif ve Sertleştirme]
    A[Kurumsal Veritabanı / Eski Anahtarlar] -->|1. Kripto Keşfini Çalıştır| B{Zafiyet Taraması}
    B -->|Savunmasız RSA / ECC| C[SOPS + age Post-Kuantum Sarmalayıcısını Konuşlandır]
    B -->|Uyumlu| D[NIST ML-KEM Anahtar Değişimini Zorunlu Kıl]
  end
  subgraph Hybrid_Compute_Stack_Integration [Hibrit Hesaplama Yığını Entegrasyonu]
    D -->|2. Classiq / Q-CTRL ile Derle| E[Kurumsal İş Yükü Yönlendiricisi]
    E -->|Klasik Katman| F[Hızlandırılmış AI / GPU Kümeleri]
    E -->|Kuantum Katman| G[NISQ / Fotonik / Nötr Atom Donanımı]
    F <-->|3. Kuantum-Klasik Döngü| G
  end
  subgraph Verified_Output [Doğrulanmış Çıktı]
    F -->|4. Doğrulanmış Sonuçları Sun| H[Yapılandırılmış ISO 20022 Verisi / API]
    H -->|5. Değiştirilemez Günlük| I[DORA Uyumluluk Kayıt Defteri]
  end
```
