...
<p class="post-lead-tldr"><strong>Özet.</strong> HTML Generator, derleme aşamasında WCAG 2.1 AA standartlarını zorunlu kılan, şema uyumlu JSON-LD enjekte eden, MathML ve Mermaid SVG'yi yerel olarak işleyen ve ayrıştırmayı bir WebAssembly sandbox ortamında yalıtan, saf Rust ile yazılmış bir Markdown'dan HTML'e derleyicisidir — yayımlamayı erişilebilirlik, SEO ve BİT güvenliği için derleme kapılı bir kontrol düzlemine dönüştürür.</p>
...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[Giriş ve AST ayrıştırma]
        A1[Ham Markdown ve YAML ön madde] --> B(HTML Generator derleyici çekirdeği)
        B --> C{WASM sandbox sınırı}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[Güvenli derleme kapıları]
        C --> D{HTML AST ayrıştırıcı}
        D -->|WCAG kuralı ihlali: düz başlıklar veya eksik alt metin| E[Derleyiciyi durdur ve derlemeyi başarısız kıl]
        D -->|WCAG 2.1 AA uyumlu| F[Zengin içerik derleyicisi]
    end

    %% Hardening & Output
    subgraph Enrichment[Zenginleştirme ve derleme]
        F -->|Matematiği MathML'e derle| G[MathML üretici]
        F -->|Diyagramları SVG'ye işle| H[Mermaid SVG üretici]
        F -->|Meta veriyi çıkar| I[JSON-LD şema enjektörü]
        G --> J[Küçültülmüş, sertleştirilmiş HTML çıktısı]
        H --> J
        I --> J
    end
