...
<p class="post-lead-tldr"><strong>TL;DR.</strong> Audio Analyser is an open-source Python application that pipelines Azure Cognitive Services — batch speech-to-text, Text Analytics NLP, and Translator — through a lightweight CherryPy web interface, producing structured transcripts, sentiment scores, and translations from audio files in WAV, MP3, OGG, or FLAC format.</p>
<p class="post-lead-heading"><strong>Key takeaways</strong></p>
<ul class="post-lead-takeaways">
  <li><strong>Neural ASR engine.</strong> Azure Batch Transcription API uses a hybrid CTC/attention acoustic model that achieves word error rates below 6% on broadcast speech and supports diarisation (speaker labelling) via a single JSON flag.</li>
  <li><strong>Single-call NLP.</strong> The Azure Text Analytics API returns key phrases, named entities, sentiment scores (positive / negative / neutral / mixed with confidence per label), and language detection in one HTTP request.</li>
  <li><strong>Lightweight host.</strong> CherryPy's ~3 MB footprint handles file upload, async transcription polling, and template rendering on a single Python process — no container orchestration required.</li>
  <li><strong>135-language translation.</strong> Azure Translator Neural Machine Translation covers 135 languages and dialects, applied after transcription with language auto-detection so no source-language flag is needed.</li>
</ul>
