...
![HSBC 양자 기술 글로벌 책임자 Philip Intallura가 Economist Impact 제5회 Commercialising Quantum Global 2026 정상회의 무대에 서 있는 모습](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [암호 발견 및 강화]
    A[기업 데이터베이스 / 레거시 키] -->|1. 암호 발견 실행| B{취약점 스캔}
    B -->|취약한 RSA / ECC| C[SOPS + age 양자내성 래퍼 배치]
    B -->|준수| D[NIST ML-KEM 키 교환 시행]
  end
  subgraph Hybrid_Compute_Stack_Integration [하이브리드 컴퓨트 스택 통합]
    D -->|2. Classiq / Q-CTRL로 컴파일| E[기업 워크로드 라우터]
    E -->|고전 계층| F[가속화된 AI / GPU 클러스터]
    E -->|양자 계층| G[NISQ / 광자 / 중성 원자 하드웨어]
    F <-->|3. 양자-고전 루프| G
  end
  subgraph Verified_Output [검증된 출력]
    F -->|4. 검증된 결과 렌더링| H[구조화 ISO 20022 데이터 / API]
    H -->|5. 변경 불가 로그| I[DORA 컴플라이언스 레지스트리]
  end
```
