...
![Philip Intallura, Global Head of Quantum Technologies de HSBC, en el escenario de la 5.ª Cumbre Anual Commercialising Quantum Global 2026 de Economist Impact](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Descubrimiento y hardening criptográfico]
    A[Base de datos corporativa / claves legadas] -->|1. Ejecutar descubrimiento cripto| B{Escaneo de vulnerabilidades}
    B -->|RSA / ECC vulnerables| C[Desplegar wrapper postcuántico SOPS + age]
    B -->|Conforme| D[Imponer intercambio de claves NIST ML-KEM]
  end
  subgraph Hybrid_Compute_Stack_Integration [Integración del stack de cómputo híbrido]
    D -->|2. Compilar vía Classiq / Q-CTRL| E[Router de cargas empresariales]
    E -->|Capa clásica| F[Clusters de IA acelerada / GPU]
    E -->|Capa cuántica| G[Hardware NISQ / fotónica / átomos neutros]
    F <-->|3. Bucle cuántico-clásico| G
  end
  subgraph Verified_Output [Salida verificada]
    F -->|4. Renderizar resultados verificados| H[Datos estructurados ISO 20022 / API]
    H -->|5. Log inalterable| I[Registro de cumplimiento DORA]
  end
```
