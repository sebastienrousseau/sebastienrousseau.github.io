...
<p class="post-lead-tldr"><strong>TL;DR.</strong> HTML Generator jẹ́ ẹ̀rọ-àkójọ Markdown-sí-HTML tí ó jẹ́ Rust funfun tí ó fi WCAG 2.1 AA ṣe dandan ní àkókò kíkọ́, tí ó fi schema-compliant JSON-LD sínú, tí ó ṣe MathML àti Mermaid SVG ní ìbínú, àti tí ó ṣe ìsọ̀ di àpótí ìfipamọ́ nínú WebAssembly sandbox — tí ó yí àgbéjáde àjọ padà sí ìlànà ìdarí compile-gated fún ìráàyèsí, SEO, àti ààbò ICT.</p>
...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[Ìgbàwọ àti àtúpalẹ̀ AST]
        A1[Markdown gọ́gọ́ àti YAML front matter] --> B(Ipilẹ̀ ẹ̀rọ-àkójọ HTML Generator)
        B --> C{Ìpẹ̀kún sandbox WASM}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[Àwọn ẹnu-ọ̀nà kíkọ́ tí ó ní ààbò]
        C --> D{Àtúpalẹ̀ HTML AST}
        D -->|Òfin WCAG lọ lòdì: orí papọ̀ tàbí ọ̀rọ̀ alt tí kò sí| E[Dáàdúró ẹ̀rọ-àkójọ àti kíkọ́ kúnfailẹ̀]
        D -->|WCAG 2.1 AA ní ibamu| F[Ẹ̀rọ-àkójọ akoonu-ọlọrọ]
    end

    %% Hardening & Output
    subgraph Enrichment[Ìdásílẹ̀ àti ìkọ́]
        F -->|Kọ́ ìṣirò sí MathML| G[Àgbéjáde MathML]
        F -->|Ṣe àwòrán sí SVG| H[Àgbéjáde Mermaid SVG]
        F -->|Yọ metadata jáde| I[Àmì ìfipamọ́ ètò JSON-LD]
        G --> J[Ìṣípò HTML tí a mìnẹ̀, tí ó mú ìlera]
        H --> J
        I --> J
    end
