...
<p class="post-lead-tldr"><strong>TL;DR。</strong> HTML Generator 是一个纯 Rust Markdown 转 HTML 编译器，在构建时强制执行 WCAG 2.1 AA 合规性，注入符合 Schema.org 规范的 JSON-LD，原生渲染 MathML 和 Mermaid SVG，并在 WebAssembly 沙盒内隔离解析——将发布工作转变为可访问性、SEO 与 ICT 安全的编译门控管理平面。</p>
...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[摄取与 AST 解析]
        A1[原始 Markdown 加 YAML 前置元数据] --> B(HTML Generator 编译器核心)
        B --> C{WASM 沙盒边界}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[安全构建门控]
        C --> D{HTML AST 解析器}
        D -->|WCAG 规则违反：扁平标题或缺失替代文本| E[停止编译并使构建失败]
        D -->|WCAG 2.1 AA 合规| F[富内容编译器]
    end

    %% Hardening & Output
    subgraph Enrichment[丰富化与编译]
        F -->|将数学公式编译为 MathML| G[MathML 生成器]
        F -->|将图表渲染为 SVG| H[Mermaid SVG 生成器]
        F -->|提取元数据| I[JSON-LD 模式注入器]
        G --> J[压缩强化的 HTML 输出]
        H --> J
        I --> J
    end
