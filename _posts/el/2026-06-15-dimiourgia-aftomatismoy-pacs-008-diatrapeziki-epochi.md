...
## Γιατί έφτιαξα το pacs008 έτσι

Έγραψα το `pacs008` και το ανάντη αδελφικό του `pain001`, και αφιερώνω την
επαγγελματική μου ζωή στις πληρωμές χονδρικής και στη διαχείριση προϊόντος API.
Αυτός ο συνδυασμός εξηγεί τη μορφή της βιβλιοθήκης, οπότε αξίζει να διατυπωθούν
οι αποφάσεις ρητά αντί να μένουν υπονοούμενες μέσα στον κώδικα.

**Η επικύρωση εκτελείται πριν από την παραγωγή, όχι μετά.** Το ένστικτο στα
περισσότερα περιβάλλοντα πληρωμών είναι να παραχθεί το μήνυμα και έπειτα να
ελεγχθεί, γιατί έτσι είναι διαμορφωμένη η διαδικασία ελέγχου: παράγεται ένα
αρχείο, κάποιος το επιθεωρεί, οι εξαιρέσεις πηγαίνουν σε ουρά. Αυτή η σειρά
εγγυάται ότι τα ελαττώματα εντοπίζονται στο σημείο με τη μικρότερη μόχλευση —
αφού η δουλειά της σύνθεσης του μηνύματος έχει ήδη γίνει, και συχνά αφού το
μήνυμα έχει ήδη φύγει από το ίδρυμα. Η επικύρωση πρώτα σημαίνει ότι μια μη
συμμορφούμενη διεύθυνση ή ένα κακοσχηματισμένο IBAN γίνεται σφάλμα build αντί για
απόρριψη στο δίκτυο.

**Η άδεια είναι MIT επειδή οι τράπεζες πρέπει να διαβάζουν τη λογική επικύρωσης,
όχι να την εμπιστεύονται.** Ένας κλειστός επικυρωτής ζητά από ένα ίδρυμα να
δεχθεί καλόπιστα την ανάγνωση κάποιου άλλου για τις οδηγίες χρήσης. Δεν είναι
εύλογο αίτημα προς μια ομάδα που φέρει την κανονιστική υποχρέωση. Κάθε κανόνας
της βιβλιοθήκης μπορεί να επιθεωρηθεί, να αμφισβητηθεί και να διακλαδωθεί.

**Η επικύρωση γίνεται έναντι των επίσημων σχημάτων XSD αντί να
επανυλοποιείται.** Μια χειρόγραφη προσέγγιση των κανόνων ISO 20022 αποκλίνει από
το πραγματικό συμβόλαιο του δικτύου μόλις αλλάξει οποιαδήποτε πλευρά. Τα σχήματα
είναι το συμβόλαιο· οτιδήποτε άλλο είναι μια δεύτερη πηγή αλήθειας που περιμένει
να διαφωνήσει.

**Στοχεύει στο CI και όχι σε ένα βήμα επιθεώρησης.** Ένας επικυρωτής που κάποιος
πρέπει να θυμηθεί να τρέξει είναι ένας επικυρωτής που παύει να τρέχει υπό την
πίεση των προθεσμιών — ακριβώς τότε που μετράει περισσότερο.

Αυτό που η βιβλιοθήκη σκόπιμα δεν κάνει είναι εξίσου σκόπιμο. Είναι εργαλειοθήκη
του επιπέδου μηνυμάτων. Δεν αντικαθιστά μηχανή πληρωμών, σύστημα ελέγχου
κυρώσεων, ούτε την εξυγίανση των κύριων δεδομένων πελάτη που το ίδρυμα οφείλει να
κάνει στην πηγή. Καθιστά αυτή την εξυγίανση απαιτητή· δεν την κάνει για εσάς.
