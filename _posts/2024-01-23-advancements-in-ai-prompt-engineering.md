...
date: "Jan 23, 2024"
description: "Prompt engineering controls LLM behaviour at inference time. This article covers zero-shot and few-shot prompting, chain-of-thought reasoning, self-consistency sampling, ReAct tool-use architecture, indirect prompt injection risks, and applied patterns from financial services deployments."
...
item_description: "Prompt engineering controls LLM behaviour at inference time. This article covers zero-shot and few-shot prompting, chain-of-thought reasoning, self-consistency sampling, ReAct tool-use architecture, indirect prompt injection risks, and applied patterns from financial services deployments."
...
item_title: "AI Prompt Engineering 2024: Techniques That Work"
...
twitter_site: "@wwdseb"
twitter_title: "AI Prompt Engineering 2024: Techniques That Work"
...
<p class="post-lead-tldr"><strong>TL;DR.</strong> Prompt engineering controls LLM behaviour at inference time by structuring the input text — no weight updates, no retraining. The techniques that proved reliable in 2024 are zero-shot task framing, chain-of-thought reasoning, self-consistency sampling, and the ReAct agent loop; the principal new risk is indirect prompt injection via RAG pipelines.</p>
<p class="post-lead-heading"><strong>Key takeaways</strong></p>
<ul class="post-lead-takeaways">
  <li><strong>Zero-shot scales with model size.</strong> Brown et al. (2020) showed that GPT-3 could match fine-tuned baselines on several NLP benchmarks with no in-context examples — but reliable zero-shot performance requires models above ~100B parameters for complex tasks.</li>
  <li><strong>Chain-of-thought needs only four words.</strong> Appending "Let's think step by step" (Kojima et al., 2022) is sufficient for zero-shot CoT; it produces gains only for tasks requiring ≥2 reasoning steps and models above ~100B parameters (Wei et al., 2022).</li>
  <li><strong>Self-consistency trades compute for accuracy.</strong> Sampling 40 independent CoT paths and majority-voting the answer raised GPT-3's GSM8K benchmark score from 56% to 74% (Wang et al., 2022).</li>
  <li><strong>Indirect prompt injection is the agent-era threat.</strong> Adversarial instructions embedded in retrieved documents can override system-prompt constraints in RAG pipelines — a documented attack class formalised by Greshake et al. (2023) that directly affects financial services deployments.</li>
</ul>
<p class="post-lead-related"><strong>Related reading:</strong> <a href="https://sebastienrousseau.com/2024-01-01-ai-trends-2024-insights-and-predictions-for-the-future/">AI Trends 2024: Insights and Predictions for the Future</a>, <a href="https://sebastienrousseau.com/2023-12-25-revolutionising-finance-with-ai-enhanced-quantum-algorithms/">Revolutionising Finance with AI-Enhanced Quantum Algorithms</a>, <a href="https://sebastienrousseau.com/2024-02-13-eus-ai-act-shaping-the-future-of-global-ai-regulation/">EU's AI Act: Pioneering Ethical AI Regulation Worldwide</a>.</p>
</aside>
<!-- lead-end -->
...
- **System** — sets model behaviour, persona, and constraints; not visible to the end user
- **User** — the end user's input
- **Assistant** — previous model turns (used to maintain conversational context)
