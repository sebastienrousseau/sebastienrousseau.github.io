...
![Philip Intallura, Global Head of Quantum Technologies do HSBC, no palco do summit Economist Impact 5th Annual Commercialising Quantum Global 2026](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Descoberta criptográfica e endurecimento]
    A[Banco de dados corporativo / chaves legadas] -->|1. Executar descoberta criptográfica| B{Varredura de vulnerabilidades}
    B -->|RSA / ECC vulneráveis| C[Implantar wrapper pós-quântico SOPS + age]
    B -->|Compatível| D[Impor troca de chaves NIST ML-KEM]
  end
  subgraph Hybrid_Compute_Stack_Integration [Integração do stack de computação híbrida]
    D -->|2. Compilar via Classiq / Q-CTRL| E[Roteador de cargas corporativas]
    E -->|Camada clássica| F[IA acelerada / clusters de GPU]
    E -->|Camada quântica| G[Hardware NISQ / fotônica / átomos neutros]
    F <-->|3. Loop quântico-clássico| G
  end
  subgraph Verified_Output [Saída verificada]
    F -->|4. Renderizar resultados verificados| H[Dados estruturados ISO 20022 / API]
    H -->|5. Log imutável| I[Registro de compliance DORA]
  end
```
