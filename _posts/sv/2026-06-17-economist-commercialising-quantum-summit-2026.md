...
![Philip Intallura, Global Head of Quantum Technologies på HSBC, på scenen vid Economist Impacts 5:e årliga Commercialising Quantum Global 2026-toppmöte](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Kryptografisk inventering och härdning]
    A[Företagsdatabas / äldre nycklar] -->|1. Kör kryptoinventering| B{Sårbarhetsskanning}
    B -->|Sårbar RSA / ECC| C[Driftsätt SOPS + age postkvantwrapper]
    B -->|Efterlevnadskompatibel| D[Tvinga NIST ML-KEM-nyckelutbyte]
  end
  subgraph Hybrid_Compute_Stack_Integration [Integration av hybrid beräkningsstack]
    D -->|2. Kompilera via Classiq / Q-CTRL| E[Router för företagsarbetslaster]
    E -->|Klassiskt lager| F[Accelererad AI / GPU-kluster]
    E -->|Kvantlager| G[NISQ / fotonik / neutral-atomhårdvara]
    F <-->|3. Kvant-klassisk loop| G
  end
  subgraph Verified_Output [Verifierad utdata]
    F -->|4. Rendera verifierade resultat| H[Strukturerad ISO 20022-data / API]
    H -->|5. Oföränderlig logg| I[DORA-efterlevnadsregister]
  end
```
