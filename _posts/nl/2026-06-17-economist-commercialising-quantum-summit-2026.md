...
![Philip Intallura, Global Head of Quantum Technologies bij HSBC, op het podium tijdens de Economist Impact 5th Annual Commercialising Quantum Global 2026-top](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Cryptografische discovery & verharding]
    A[Bedrijfsdatabase / legacy-sleutels] -->|1. Crypto-discovery uitvoeren| B{Kwetsbaarheidsscan}
    B -->|Kwetsbaar RSA / ECC| C[SOPS + age post-kwantumwrapper uitrollen]
    B -->|Compliant| D[NIST ML-KEM-sleuteluitwisseling afdwingen]
  end
  subgraph Hybrid_Compute_Stack_Integration [Integratie hybride compute-stack]
    D -->|2. Compileren via Classiq / Q-CTRL| E[Enterprise workload router]
    E -->|Klassieke laag| F[Versnelde AI / GPU-clusters]
    E -->|Kwantumlaag| G[NISQ / fotonica / neutrale-atomenhardware]
    F <-->|3. Kwantum-klassieke lus| G
  end
  subgraph Verified_Output [Geverifieerde output]
    F -->|4. Geverifieerde resultaten weergeven| H[Gestructureerde ISO 20022-data / API]
    H -->|5. Onveranderlijk log| I[DORA-complianceregister]
  end
```
