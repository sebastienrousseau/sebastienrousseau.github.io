...
## Почему я построил pacs008 именно так

Я написал `pacs008` и его старшего собрата `pain001`, а моя профессиональная
жизнь связана с оптовыми платежами и продуктовым управлением API. Именно это
сочетание объясняет облик библиотеки, поэтому решения стоит изложить прямо, а не
оставлять их неявными в коде.

**Проверка выполняется до генерации, а не после.** В большинстве платёжных
ландшафтов рефлекс таков: сначала сформировать сообщение, потом его проверить —
потому что так устроен процесс контроля: файл создаётся, кто-то его просматривает,
исключения уходят в очередь. Такой порядок гарантирует, что дефекты находят в
точке наименьшего рычага — когда работа по сборке сообщения уже сделана, а часто
и после того, как оно покинуло организацию. Проверять сначала означает, что
несоответствующий адрес или некорректный IBAN становится ошибкой сборки, а не
отказом сети.

**Лицензия MIT, потому что банки должны читать логику проверки, а не доверять
ей.** Закрытый валидатор просит организацию принять на веру чужое прочтение
руководств по применению. Это неразумно требовать от команды, которая несёт
регуляторную обязанность. Любое правило этой библиотеки можно изучить, оспорить и
форкнуть.

**Проверка идёт против официальных XSD-схем, а не против собственной
переимплементации.** Написанное вручную приближение правил ISO 20022 расходится с
фактическим контрактом сети, как только меняется любая из сторон. Схемы и есть
контракт; всё остальное — второй источник истины, ожидающий противоречия.

**Она нацелена на CI, а не на шаг ревью.** Валидатор, о запуске которого человеку
нужно помнить, — это валидатор, который перестают запускать под давлением сроков,
то есть ровно тогда, когда он важнее всего.

То, чего библиотека сознательно не делает, столь же осознанно. Это инструментарий
уровня сообщений. Он не заменяет платёжный движок, систему санкционного скрининга
или приведение в порядок мастер-данных клиента, которое организация обязана
выполнить у источника. Он делает эту работу проверяемой; он не выполняет её за
вас.
