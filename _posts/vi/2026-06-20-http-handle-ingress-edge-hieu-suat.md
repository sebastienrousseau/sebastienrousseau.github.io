...
<p class="post-lead-tldr"><strong>TL;DR.</strong> http-handle là một tệp nhị phân Rust nguồn mở, liên kết tĩnh, thay thế Nginx và Envoy tại cạnh ngân hàng. Nó cung cấp 180.000 yêu cầu mỗi giây trên các nút ARM64 qua truyền tải zero-copy <code>sendfile(2)</code> của Linux, thực thi mTLS và xác thực JWT tại lớp socket mạng trước khi bất kỳ mã ứng dụng nào chạy, tự động đàm phán HTTP/2 và HTTP/3 qua ALPN và phát ra chỉ số OpenTelemetry tự nhiên — tất cả với không có phụ thuộc thời gian chạy ngoài <code>libc</code>.</p>
