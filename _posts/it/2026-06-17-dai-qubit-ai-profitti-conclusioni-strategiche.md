...
![Philip Intallura, Global Head of Quantum Technologies di HSBC, sul palco del summit Economist Impact 5th Annual Commercialising Quantum Global 2026](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Discovery crittografica e hardening]
    A[Database aziendale / chiavi legacy] -->|1. Esegui crypto discovery| B{Scan delle vulnerabilità}
    B -->|RSA / ECC vulnerabili| C[Distribuisci wrapper post-quantistico SOPS + age]
    B -->|Conforme| D[Imponi key exchange NIST ML-KEM]
  end
  subgraph Hybrid_Compute_Stack_Integration [Integrazione dello stack di calcolo ibrido]
    D -->|2. Compila tramite Classiq / Q-CTRL| E[Router dei carichi di lavoro aziendali]
    E -->|Strato classico| F[Cluster AI accelerati / GPU]
    E -->|Strato quantistico| G[Hardware NISQ / fotonica / atomi neutri]
    F <-->|3. Loop quantum-classical| G
  end
  subgraph Verified_Output [Output verificato]
    F -->|4. Rendi risultati verificati| H[Dati strutturati ISO 20022 / API]
    H -->|5. Log immutabile| I[Registro di conformità DORA]
  end
```
