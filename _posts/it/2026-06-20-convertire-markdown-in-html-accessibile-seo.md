...
<p class="post-lead-tldr"><strong>TL;DR.</strong> HTML Generator è un compilatore Markdown-to-HTML in puro Rust che impone WCAG 2.1 AA in fase di compilazione, inietta JSON-LD conforme agli schemi, esegue il rendering nativo di MathML e SVG Mermaid, e isola il parsing in una sandbox WebAssembly — trasformando la pubblicazione in un piano di controllo compilativo per accessibilità, SEO e sicurezza ICT.</p>
...
<p class="post-lead-related"><strong>Letture correlate:</strong> <a href="https://sebastienrousseau.com/2026-06-18-noyalib-safe-yaml-rust-ai-mcp-financial-infrastructure-2026">Perché YAML richiede uno stack Rust più sicuro per AI, MCP e infrastruttura finanziaria nel 2026</a>, Un Static Site Generator sicuro per impostazione predefinita per la pubblicazione nell'era AI nel 2026, <a href="https://sebastienrousseau.com/2026-06-11-cloudcdn-open-source-blueprint-ai-native-edge-2026">CloudCDN: un modello open source per l'edge AI-native nel 2026</a>.</p>
</aside>
<!-- lead-end -->
...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[Ingestione e parsing AST]
        A1[Markdown grezzo con front matter YAML] --> B(Nucleo compilatore HTML Generator)
        B --> C{Confine sandbox WASM}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[Blocchi di build sicuri]
        C --> D{Parser HTML AST}
        D -->|Regola WCAG violata: intestazioni piatte o alt text mancante| E[Interrompi compilatore e fallisci la build]
        D -->|Conforme a WCAG 2.1 AA| F[Compilatore contenuti avanzati]
    end

    %% Hardening & Output
    subgraph Enrichment[Arricchimento e compilazione]
        F -->|Compila matematica in MathML| G[Generatore MathML]
        F -->|Rendering diagrammi in SVG| H[Generatore SVG Mermaid]
        F -->|Estrai metadati| I[Iniettore schema JSON-LD]
        G --> J[Output HTML minificato e rafforzato]
        H --> J
        I --> J
    end
