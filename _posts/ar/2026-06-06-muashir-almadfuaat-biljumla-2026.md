...
sequenceDiagram
    autonumber
    participant CB_A as المصرف التجاري A
    participant UL as السجل الموحَّد
    participant CBNK as المصرف المركزي
    participant CB_B as المصرف التجاري B
    CB_A->>UL: تقديم التعليمات: خصم وديعة مرمَّزة X، وقيد وديعة مرمَّزة Y، الشرط = ساق CBDC بالجملة
    UL->>UL: التحقق من مغلَّف ISO 20022، والعنوان المهيكل، وLEI، ورمز الغرض، وAML/العقوبات
    UL->>CBNK: حجز CBDC بالجملة (احتياطيات المصرف المركزي لجانب الدافع)
    CBNK-->>UL: تأكيد الحجز (قفل ذري)
    UL->>CB_A: قفل وديعة مرمَّزة X (ساق نقود المصرف التجاري)
    CB_A-->>UL: تأكيد قفل الوديعة
    UL->>UL: قُفلت كلتا الساقين → تحفيز التسوية الذرية
    UL->>CBNK: تسوية CBDC بالجملة (احتياطيات الدافع → احتياطيات المستفيد)
    UL->>CB_B: إصدار وديعة مرمَّزة Y (ساق نقود المصرف التجاري)
    CBNK-->>UL: تسوية CBDC نهائية
    CB_B-->>UL: قُيِّدت الوديعة
    UL->>CB_A: تسوية PvP مكتملة (الساقان نهائيتان أو تتراجعان معًا)
