...
| **التحكم بوصول الوكيل** | صلاحيات مقيَّدة للقراءة فقط لأدوات الذكاء الاصطناعي المحلية العاملة عبر تهيئات MCP. | إدارة مخاطر النماذج ([SR 11-7](https://web.archive.org/web/20260414150921/https://www.federalreserve.gov/supervisionreg/srletters/sr1107.htm)) | حدود خوادم MCP تُقيِّد عمليات الوكلاء بالأدلة المعتمدة. |
...
```mermaid
graph TD
  subgraph Raw_Manifest_Ingestion [استيعاب المانيفست الخام]
    A1[مستودع GitHub / YAML 1.2] -->|1. جلب التهيئة| B(مُحلِّل NoyaLib)
    A2[وكيل ذكاء اصطناعي / أداة إعادة هيكلة آلية] -->|2. اقتراح تغيير محلي| B
  end
  subgraph NoyaLib_Core_Parser [جوهر مُحلِّل NoyaLib]
    B -->|3. تحليل بصفر كتل Unsafe| C{مُولِّد CST عديم الفقد}
    C -->|4. بناء CST يحفظ التعليقات والمسافات| D[شجرة CST]
  end
  subgraph Schema_Validation_Gate [بوابة التحقق من المخطَّط]
    D -->|5. استخراج AST| E[مُتحقِّق JSON-Schema]
    E -->|انتهاك المخطَّط / نوع غير سليم| F[إيقاف الخط ورفض التغيير]
    E -->|تحقق من المخطَّط 100%| G[مُترجم WASM / موقِّع GPG]
  end
  subgraph Secure_Cloud_Native_Deployment [نشر سحابي أصيل آمن]
    G -->|6. ترجمة YAML المُتحقَّق إلى WASM / JSON| H[عنقود Kubernetes / محرك CI]
    G -->|7. إلحاق سجل تدقيق| I[سجل تشغيلي ثابت]
  end
```
