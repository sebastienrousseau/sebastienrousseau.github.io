...
sequenceDiagram
    autonumber
    participant CB_A as Комерційний банк A
    participant UL as Єдиний реєстр
    participant CBNK as Центральний банк
    participant CB_B as Комерційний банк B
    CB_A->>UL: Подати інструкцію: списати токенізований депозит X, зарахувати токенізований депозит Y, умова = leg оптової CBDC
    UL->>UL: Валідація конверту ISO 20022, структурована адреса, LEI, код призначення, AML/санкції
    UL->>CBNK: Зарезервувати оптову CBDC (резерви центрального банку на стороні платника)
    CBNK-->>UL: Резервування підтверджено (атомарне блокування)
    UL->>CB_A: Заблокувати токенізований депозит X (leg грошей комерційного банку)
    CB_A-->>UL: Блокування депозиту підтверджено
    UL->>UL: Обидва legs заблоковано → тригер атомарного розрахунку
    UL->>CBNK: Розрахунок оптовою CBDC (резерви платника → резерви отримувача)
    UL->>CB_B: Емітувати токенізований депозит Y (leg грошей комерційного банку)
    CBNK-->>UL: Розрахунок CBDC фіналізовано
    CB_B-->>UL: Депозит зараховано
    UL->>CB_A: PvP розрахунок завершено (обидва legs остаточні, або обидва відкочуються)
