...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[Інгестія та парсинг AST]
        A1[Необроблений Markdown та YAML front matter] --> B(Ядро компілятора HTML Generator)
        B --> C{Межа пісочниці WASM}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[Захищені фільтри збірки]
        C --> D{Парсер HTML AST}
        D -->|Порушення правил WCAG: пласкі заголовки або відсутній alt-текст| E[Зупинити компілятор та перервати збірку]
        D -->|WCAG 2.1 AA відповідний| F[Компілятор багатого контенту]
    end

    %% Hardening & Output
    subgraph Enrichment[Збагачення та компіляція]
        F -->|Компіляція математики в MathML| G[Генератор MathML]
        F -->|Відтворення діаграм у SVG| H[Генератор Mermaid SVG]
        F -->|Вилучення метаданих| I[Інжектор схем JSON-LD]
        G --> J[Мінімізований, захищений HTML-вивід]
        H --> J
        I --> J
    end
