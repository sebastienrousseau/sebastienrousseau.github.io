...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [暗号の発見と堅牢化]
    A[企業データベース / レガシー鍵] -->|1. 暗号発見を実行| B{脆弱性スキャン}
    B -->|脆弱な RSA / ECC| C[SOPS + age ポスト量子ラッパーを展開]
    B -->|適合| D[NIST ML-KEM 鍵交換を強制]
  end
  subgraph Hybrid_Compute_Stack_Integration [ハイブリッドコンピュートスタック統合]
    D -->|2. Classiq / Q-CTRL でコンパイル| E[エンタープライズワークロードルーター]
    E -->|古典層| F[加速 AI / GPU クラスター]
    E -->|量子層| G[NISQ / フォトニクス / 中性原子ハードウェア]
    F <-->|3. 量子古典ループ| G
  end
  subgraph Verified_Output [検証済み出力]
    F -->|4. 検証済み結果をレンダリング| H[構造化 ISO 20022 データ / API]
    H -->|5. 改ざん不能ログ| I[DORA コンプライアンス・レジストリ]
  end
```
