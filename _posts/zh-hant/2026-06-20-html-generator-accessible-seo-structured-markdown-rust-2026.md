...
<p class="post-lead-tldr"><strong>TL;DR。</strong> HTML Generator 是一個純 Rust 的 Markdown 轉 HTML 編譯器，在建置時強制執行 WCAG 2.1 AA，自動注入符合 Schema.org 規範的 JSON-LD，原生渲染 MathML 與 Mermaid SVG，並在 WebAssembly 沙盒中隔離解析——將發布流程轉變為兼顧無障礙、SEO 與資訊安全的編譯閘控平台。</p>
...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[擷取與 AST 解析]
        A1[原始 Markdown 加 YAML 前置資料] --> B(HTML Generator 編譯器核心)
        B --> C{WASM 沙盒邊界}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[安全建置閘控]
        C --> D{HTML AST 解析器}
        D -->|WCAG 規則違反：扁平標題或缺少替代文字| E[停止編譯並建置失敗]
        D -->|WCAG 2.1 AA 合規| F[豐富內容編譯器]
    end

    %% Hardening & Output
    subgraph Enrichment[豐富化與編譯]
        F -->|將數學式編譯為 MathML| G[MathML 產生器]
        F -->|將圖表渲染為 SVG| H[Mermaid SVG 產生器]
        F -->|提取元資料| I[JSON-LD 綱要注入器]
        G --> J[最小化、強化的 HTML 輸出]
        H --> J
        I --> J
    end
