...
<p class="post-lead-tldr"><strong>TL;DR.</strong> HTML Generator adalah kompiler Markdown-ke-HTML berbasis Rust murni yang menegakkan WCAG 2.1 AA pada waktu kompilasi, menyuntikkan JSON-LD yang sesuai skema, merender MathML dan Mermaid SVG secara native, serta mengisolasi penguraian di dalam sandbox WebAssembly — mengubah penerbitan menjadi bidang kontrol yang dikontrol kompiler untuk aksesibilitas, SEO, dan keamanan TIK.</p>
...
graph TD
    %% Ingest & Parse
    subgraph Ingest_AST[Ingest dan penguraian AST]
        A1[Markdown mentah plus YAML front matter] --> B(Inti kompiler HTML Generator)
        B --> C{Batas sandbox WASM}
    end

    %% Build Gates
    subgraph Secure_Build_Gates[Gerbang kompilasi aman]
        C --> D{Pengurai HTML AST}
        D -->|Aturan WCAG dilanggar: header datar atau alt text hilang| E[Hentikan kompiler dan gagalkan kompilasi]
        D -->|WCAG 2.1 AA patuh| F[Kompiler konten kaya]
    end

    %% Hardening & Output
    subgraph Enrichment[Pengayaan dan kompilasi]
        F -->|Kompilasi matematika ke MathML| G[Generator MathML]
        F -->|Render diagram ke SVG| H[Generator Mermaid SVG]
        F -->|Ekstrak metadata| I[Penyuntik skema JSON-LD]
        G --> J[Output HTML yang diperkecil dan diperkuat]
        H --> J
        I --> J
    end
