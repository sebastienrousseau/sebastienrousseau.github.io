...
```mermaid
graph TD
  subgraph Raw_Manifest_Ingestion [Ingesti Manifes Mentah]
    A1[Repositori GitHub / YAML 1.2] -->|1. Ambil Konfigurasi| B(Parser NoyaLib)
    A2[Agen AI / Alat Refactoring Otomatis] -->|2. Usulkan Perubahan Lokal| B
  end
  subgraph NoyaLib_Core_Parser [Inti Parser NoyaLib]
    B -->|3. Parse dengan Nol Blok Unsafe| C{Generator CST Lossless}
    C -->|4. Bangun CST dengan menjaga komentar & spasi| D[Concrete Syntax Tree CST]
  end
  subgraph Schema_Validation_Gate [Gerbang Validasi Skema]
    D -->|5. Ekstrak Abstract Syntax Tree AST| E[Validator JSON Schema]
    E -->|Pelanggaran Skema / Tipe Tidak Valid| F[Hentikan Pipeline & Tolak Perubahan]
    E -->|Skema Tervalidasi 100%| G[Kompiler WASM / Penandatangan GPG]
  end
  subgraph Secure_Cloud_Native_Deployment [Deployment Cloud-Native yang Aman]
    G -->|6. Kompilasi YAML Tervalidasi ke WASM / JSON| H[Klaster Kubernetes / Mesin CI]
    G -->|7. Tambahkan Log Audit| I[Ledger Operasional Tidak Dapat Diubah]
  end
```
