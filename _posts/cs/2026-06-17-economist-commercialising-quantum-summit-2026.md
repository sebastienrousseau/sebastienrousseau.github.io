...
![Philip Intallura, Global Head of Quantum Technologies v HSBC, na pódiu summitu Economist Impact 5th Annual Commercialising Quantum Global 2026](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Kryptografické objevení a zpevnění]
    A[Korporátní databáze / starší klíče] -->|1. Spustit kryptografické objevování| B{Sken zranitelnosti}
    B -->|Zranitelné RSA / ECC| C[Nasadit SOPS + age postkvantový wrapper]
    B -->|Vyhovující| D[Vynutit výměnu klíčů NIST ML-KEM]
  end
  subgraph Hybrid_Compute_Stack_Integration [Integrace hybridního výpočetního stacku]
    D -->|2. Kompilovat přes Classiq / Q-CTRL| E[Směrovač podnikových workloadů]
    E -->|Klasická vrstva| F[Akcelerované AI / GPU klastry]
    E -->|Kvantová vrstva| G[NISQ / fotonický / neutrální atom hardware]
    F <-->|3. Kvantově-klasická smyčka| G
  end
  subgraph Verified_Output [Ověřený výstup]
    F -->|4. Renderovat ověřené výsledky| H[Strukturovaná data ISO 20022 / API]
    H -->|5. Neměnný log| I[Registr compliance DORA]
  end
```
