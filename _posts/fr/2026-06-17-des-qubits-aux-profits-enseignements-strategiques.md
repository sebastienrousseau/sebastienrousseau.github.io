...
![Philip Intallura, Global Head of Quantum Technologies chez HSBC, sur scène au sommet Economist Impact 5th Annual Commercialising Quantum Global 2026](https://cloudcdn.pro/stocks/images/philip-intallura-hsbc-economist-5th-annual-commercialising-quantum.webp)
...
```mermaid
graph TD
  subgraph Cryptographic_Discovery_and_Hardening [Découverte et durcissement cryptographiques]
    A[Base corporate / clés héritées] -->|1. Lancer la découverte crypto| B{Analyse de vulnérabilités}
    B -->|RSA / ECC vulnérables| C[Déployer SOPS + wrapper post-quantique age]
    B -->|Conforme| D[Imposer l'échange de clés NIST ML-KEM]
  end
  subgraph Hybrid_Compute_Stack_Integration [Intégration de la pile de calcul hybride]
    D -->|2. Compiler via Classiq / Q-CTRL| E[Routeur de charges d'entreprise]
    E -->|Couche classique| F[Clusters IA accélérée / GPU]
    E -->|Couche quantique| G[Matériel NISQ / photonique / atomes neutres]
    F <-->|3. Boucle quantique-classique| G
  end
  subgraph Verified_Output [Sortie vérifiée]
    F -->|4. Rendre les résultats vérifiés| H[Données structurées ISO 20022 / API]
    H -->|5. Journal immuable| I[Registre de conformité DORA]
  end
```
