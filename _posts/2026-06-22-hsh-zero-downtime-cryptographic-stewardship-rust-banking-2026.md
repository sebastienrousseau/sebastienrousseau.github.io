...
banner_alt: "Close-up of a developer terminal scrolling Rust compiler output — the visible discipline of a memory-safe, audit-able cryptographic substrate beneath an enterprise banking authentication estate"
banner_height: "1597"
banner_width: "2584"
...
Under the [Digital Operational Resilience Act (DORA)](https://eur-lex.europa.eu/eli/reg/2022/2554/oj "Regulation (EU) 2022/2554 — Digital Operational Resilience Act"), leaving un-rotated, legacy cryptographic assets in production is no longer technical debt. It is named regulatory liability.
...
**What happens to dormant accounts that never log in?**
Records that never authenticate never re-hash. Banks address this with two complementary policies: a documented dormancy threshold (often 18–24 months) after which the account is administratively rotated under a controlled reset campaign, and a synthetic re-hash run during scheduled maintenance for accounts in defined cohorts (high-value, high-privilege, regulated). Both are policies, not library behaviours; hsh records the dispatch decision in audit telemetry so the operational owner can prove coverage.
...
The [hsh source code](https://github.com/sebastienrousseau/hsh "hsh — pure-Rust password hashing framework, MIT and Apache 2.0 dual licence") is available under the MIT and Apache 2.0 dual licence.
