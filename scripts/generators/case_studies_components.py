...
def _render_breadcrumb(
    ...
    return (
        f'<nav class="cs-breadcrumb" aria-label="{_esc(lbl["Home"])}">' + "".join(parts) + "</nav>"
    )
...
def _render_meta_strip(study: dict, lbl: dict[str, str]) -> str:
    ...
    for key, val in fields:
        if val:
            pieces.append(f"<li><strong>{_esc(lbl[key])}</strong> {_esc(val)}</li>")
    if not pieces:
        return ""
...
def _render_more_case_studies(
    ...
    return (
        ...
        '<div class="cs-related-grid" role="list">' + "".join(cards) + "</div></section>"
    )
...
def _render_inline_outcomes(outcomes: list[dict]) -> str:
    if not outcomes:
        return ""
    items = "".join(
        f"<div><dt>{_esc(o.get('value', ''))}</dt><dd>{_esc(o.get('label', ''))}</dd></div>"
        for o in outcomes
    )
...
def _prose_body_for(
    study: dict, lbl: dict[str, str], lang: str, article_slug_map: dict[str, str], field: str
) -> str:
