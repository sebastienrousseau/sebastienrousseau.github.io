...
#
# Usage: scripts/sigstore_setup.sh
set -euo pipefail
...
git status --short docs/sigstore/ _data/sigstore/.gitignore _data/sigstore/config.example.json doc/SIGSTORE.md 2>/dev/null | sed 's/^/    /'
echo
echo "  To publish:"
echo "    git add docs/sigstore/ _data/sigstore/.gitignore _data/sigstore/config.example.json doc/SIGSTORE.md"
echo "    git commit -m 'chore(sigstore): activate signing — per-article bundles'"
echo "    git push"
echo
echo "  Then run scripts/sigstore_setup.sh again any time you rebuild — it's idempotent."
