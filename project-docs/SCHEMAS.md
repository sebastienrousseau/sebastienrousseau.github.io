...
|---|---:|---|
| `Person` | 1850 | [`scripts/generators/build_agent_api.py`](../scripts/generators/build_agent_api.py) + per-page JSON-LD |
...
2. CI gate [`test_csp_strict.py`](../tests/validation/test_csp_strict.py) walks every rendered page, extracts every inline JSON-LD block, computes its hash, and confirms that hash appears in the page's `script-src`. Fails the build if not.
