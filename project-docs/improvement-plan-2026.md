...
- **1.3 — IN PROGRESS.** Expand coverage past the gated modules to repo-wide ≥90% / libs 100%; prioritise untested modules by blast radius. **Done:** `check_voice.py` (the publishing gate, 378 LOC, was untested) now covered by `tests/unit/test_check_voice.py` — 13 cases over the pure checks (frontmatter completeness, banned filler, EN structure, markdown discipline, date consistency). **Remaining (by blast radius):** `build_case_studies.py`, `build_listings.py`, `backfill_locale_frontmatter.py`, `regen_homepage.py`, `regen_slug_maps.py`, `build_tags.py`, `fetch_metrics.py`, `build_rag_corpus.py`, and the smaller editorial/seo modules.
