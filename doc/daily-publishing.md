...
2. Run `./scripts/publish_daily.sh` which:
   - `git mv`s draft → `_posts/`
   - calls `scripts/translate_post.py <slug>` to **scaffold 27 stub
...
3. Manually update two curated files: `_posts/index.md`
   (newsroom-grid) and `scripts/gen_articles.py` (ARTICLES[0]).
...
- Re-running `scripts/translate_post.py <slug>` only overwrites stubs
  (files still carrying the `<!-- translation-stub -->` marker). Files
  that have already been translated are left alone.
- `scripts/translate_post.py <slug> --list-stubs` prints exactly which
...
| Symptom | Fix |
|---------|-----|
| `./scripts/publish_daily.sh: no _drafts/<today>-*.md` | You haven't dropped a draft. Do that, retry. |
| `test_i18n_parity` fails on a locale | `_data/i18n/<lang>/slugs.json` didn't get the new EN→native mapping. Re-run `python3 scripts/translate_post.py <slug>`. |
...
- **No auto-edit of curated lists.** `_posts/index.md`'s newsroom-grid
  and `scripts/gen_articles.py`'s `ARTICLES` tuple are editorial
