...
The single source of truth is [`scripts/_lang_registry.py`](../scripts/_lang_registry.py):
...
sequenceDiagram
    autonumber
    participant SSG as Shokunin SSG
...
2. **`scripts/test_rtl_safe.py --strict`** — CI gate that fails the build if any layout uses physical CSS properties (`margin-left`, `padding-right`, `text-align: left`, `border-left`, etc.) that don't flip with `dir="rtl"`. Required CSS pattern is logical properties (`margin-inline-start`, `text-align: start`, `border-inline-start`).
...
Enforced by [`scripts/test_hreflang_reciprocity.py`](../scripts/test_hreflang_reciprocity.py) on every build. 1848 paired pages on a clean build.
...
|---|---|---|
| `test_i18n_parity` | Every active lang renders the same article count as EN | [`scripts/test_i18n_parity.py`](../scripts/test_i18n_parity.py) |
| `test_i18n_strings` | UI strings keyset matches EN reference (52 keys) | [`scripts/test_i18n_strings.py`](../scripts/test_i18n_strings.py) |
| `test_i18n_labels` | Body labels keyset matches EN reference (12 keys) | [`scripts/test_i18n_labels.py`](../scripts/test_i18n_labels.py) |
| `test_i18n_takeaway_labels` | Takeaway labels keyset matches EN reference (29 keys) | [`scripts/test_i18n_takeaway_labels.py`](../scripts/test_i18n_takeaway_labels.py) |
| `test_i18n_render_data` | Patch-table count matches FR canonical (home=78 static=254 chrome=71) | [`scripts/test_i18n_render_data.py`](../scripts/test_i18n_render_data.py) |
...
| `test_hreflang_reciprocity` | Symmetric hreflang pairs across all locales | [`scripts/test_hreflang_reciprocity.py`](../scripts/test_hreflang_reciprocity.py) |
| `test_jsonld_localized` | JSON-LD `inLanguage` matches `<html lang>` on every page | [`scripts/test_jsonld_localized.py`](../scripts/test_jsonld_localized.py) |
| `test_rtl_safe` | RTL languages don't use physical CSS properties | [`scripts/test_rtl_safe.py`](../scripts/test_rtl_safe.py) |
...
1. **Register** — append a `Language(...)` row to [`scripts/_lang_registry.py`](../scripts/_lang_registry.py) with `active=False` initially. RTL languages get `rtl=True`.
